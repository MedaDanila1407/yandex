...
const BurgerConstructor: FC = () => {
  ...
  const allIngredients = useAppSelector(
    (store) => store.burgerConstructorReducer.allIngredients
  );
  ...
  let allCost = constructorIngredients.reduce(
    (sum: number, currentItem: TIngredient) => {
  ...
  const [, dropTarget] = useDrop({
    ...
    drop(itemId: { id: string }) {
      const item = allIngredients.find(
        (item: TIngredient) => item._id === itemId.id
      );
      item.type === "bun"
  ...
  const moveIngredient = (
    ...
    dispatch(
      changeIngredientsSort(dragIndex, hoverIndex, constructorIngredients)
    );
  };
      ...
      <div className={styles.order}>
        ...
        <div className={styles.orderSubmit}>
          <Button
            ...
            disabled={
              constructorIngredients.length === 0 && !bun ? true : false
            }
          >
            Оформить заказ
          </Button>
